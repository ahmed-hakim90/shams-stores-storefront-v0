      ...
      <main className="mx-auto max-w-[1400px] px-4 py-8 sm:px-6 sm:py-12">
        ...
        <div className="mt-8 overflow-x-auto rounded-lg border border-border bg-card">
          <div className="grid min-w-[720px] grid-cols-4">
            ...
            {products.map((product) => <div key={`${product.id}-stock`} className="border-r border-border p-4 text-sm last:border-r-0"><span className="inline-flex items-center gap-1.5 text-success"><Check className="size-4" />{product.stockLabel ?? 'In stock'}</span><p className="mt-2 text-xs text-muted-foreground">Pickup available at selected branches</p></div>)}

...
export function CatalogPage({ category, products, title, description, query }: { category?: Category; products: Product[]; title: string; description: string; query?: string }) {
  ...
  const categoryChips = category && <div className="flex gap-2 overflow-x-auto"><Link href={`/c/${category.slug}`} className="shrink-0 rounded-full border border-brand bg-brand/5 px-4 py-2 text-sm font-medium text-brand-ink">All {category.name}</Link>{category.columns[0]?.links.slice(0, 5).map((link) => <Link key={link.href + link.label} href={link.href} className="shrink-0 rounded-full border border-border px-4 py-2 text-sm text-muted-foreground hover:border-brand hover:text-brand-ink">{link.label}</Link>)}</div>
  const listingToolbar = <div className="flex items-center justify-between gap-3"><Button variant="outline" className="min-h-11 gap-2 lg:hidden" onClick={() => setFiltersOpen(true)}><SlidersHorizontal data-icon="inline-start" />Filters{activeFilters.length > 0 && ` (${activeFilters.length})`}</Button><div className="hidden items-center gap-2 text-sm text-muted-foreground lg:flex"><SlidersHorizontal className="size-4" /> Refine your search</div><label className="flex min-h-11 items-center gap-2 text-sm text-muted-foreground">Sort<select value={sort} onChange={(event) => setSort(event.target.value)} className="min-h-11 rounded-lg border border-border bg-background px-3 font-medium text-foreground outline-none focus:border-brand"><option>Featured</option><option>Best Selling</option><option>Newest</option><option>Price: Low to High</option><option>Price: High to Low</option><option>Top Rated</option></select></label></div>

  return <main className="mobile-storefront-page mx-auto max-w-[1440px] px-4 py-4 sm:px-6 sm:py-10">
...
function FilterList({ filters, active, toggle, clear }: { filters: string[]; active: string[]; toggle: (filter: string) => void; clear: () => void }) {
  ...
  return <div className="flex flex-col">{filters.map((filter) => { const isExpanded = expanded === filter; const isActive = active.includes(filter); return <div key={filter} className="border-b border-border"><button type="button" aria-expanded={isExpanded} onClick={() => setExpanded(isExpanded ? null : filter)} className="flex min-h-14 w-full items-center justify-between py-2 text-left text-base"><span className={isActive ? 'font-medium text-brand-ink' : 'text-foreground'}>{filter}</span><ChevronDown className={`size-5 text-muted-foreground transition-transform ${isExpanded ? 'rotate-180' : ''}`} /></button>{isExpanded && <div className="flex items-center justify-between gap-3 pb-3"><p className="text-sm text-muted-foreground">Choose {filter.toLowerCase()} filters</p><button type="button" onClick={() => toggle(filter)} className={`min-h-11 rounded-lg border px-3 text-sm font-medium ${isActive ? 'border-brand bg-brand/10 text-brand-ink' : 'border-border hover:border-brand'}`}>{isActive ? 'Selected' : 'Select'}</button></div>}</div>})}{active.length > 0 && <button type="button" onClick={clear} className="mt-3 min-h-11 text-left text-sm text-brand underline">Clear all filters</button>}</div>
}

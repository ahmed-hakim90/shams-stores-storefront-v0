...
export function Footer() {
  return (
    <footer className="border-t border-border bg-surface-dark text-on-dark">
      <div className="mx-auto max-w-[1440px] px-4 py-12 sm:px-6">
        <div className="grid gap-10 lg:grid-cols-[1.4fr_repeat(3,1fr)]">
          <div className="space-y-4">
            ...
            <div className="space-y-1.5 text-sm text-on-dark-muted">
              <p className="flex items-center gap-2">
                <MapPin className="size-4 text-brand" /> Downtown · Heliopolis · Alexandria
              </p>
              <p className="flex items-center gap-2">
                <Phone className="size-4 text-brand" /> +201011331666
              </p>
              <p className="flex items-center gap-2">
          ...
          {columns.map((col) => (
            <div key={col.heading}>
              <p className="mb-3 text-sm font-semibold text-on-dark">{col.heading}</p>
              <ul className="space-y-2">
                {col.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-on-dark-muted transition-colors duration-fast hover:text-on-dark"
                    >
                      {link.label}
                    </Link>
                  </li>
      ...
      <div className="border-t border-on-dark-border">
        <div className="mx-auto flex max-w-[1440px] flex-col items-center justify-between gap-3 px-4 py-5 pb-[max(1.25rem,calc(1.25rem+var(--mobile-bottom-nav-height)))] text-xs text-on-dark-muted sm:flex-row sm:px-6 sm:pb-5">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-center gap-1.5">
              {['VISA', 'Mastercard', 'ValU', 'Fawry'].map((m) => (
                <span
                  key={m}
                  className="rounded border border-on-dark-border bg-white/5 px-1.5 py-0.5 text-[0.65rem] font-medium text-on-dark-muted"
                >
                  {m}
                </span>
              ))}
            ...
            <div className="flex items-center gap-4">
              <Link href="/terms" className="transition-colors hover:text-on-dark">Terms & conditions</Link>
              <Link href="/privacy" className="transition-colors hover:text-on-dark">Privacy policy</Link>
              <Link href="/cookies" className="transition-colors hover:text-on-dark">Cookie policy</Link>
            </div>
          </div>
        </div>
      </div>

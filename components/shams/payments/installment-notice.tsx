...
export function InstallmentNotice({ eligible, href, detailed = false }: {
  ...
  if (!eligible || query.isError || !query.data?.paymob || !query.data.paymobOptions?.some(option => option.kind === 'installments')) return null
  if (detailed) return (
    <aside id="installments" className="mt-3 rounded-(--radius-control) border border-brand/20 bg-brand/5 p-3 scroll-mt-24" aria-label="Bank installment information">
      ...
      <p className="mt-1.5 text-xs leading-5 text-muted-foreground">Select Bank installments at checkout to explore eligible banks and plans without leaving Shams. Your bank determines eligibility, durations and any fees before you confirm.</p>
    </aside>
  )

...
export function CatalogDiscovery({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <section className={compact ? 'mb-8' : 'mx-auto max-w-[1400px] px-4 py-12 sm:px-6 sm:py-16'}>
      ...
      <div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        {categories.map((category) => {
          ...
          return <Link key={category.slug} href={`/c/${category.slug}`} className="group relative min-h-36 overflow-hidden rounded-xl border border-border bg-card p-4 transition-colors hover:border-brand/50">
            ...
            <div className="relative flex h-full min-h-28 flex-col justify-end"><p className="text-xs font-semibold uppercase tracking-widest text-brand">{category.itemCount} products</p><h3 className="mt-1 text-lg font-semibold">{category.name}</h3><p className="mt-1 line-clamp-1 text-xs text-muted-foreground">{category.tagline}</p><span className="mt-3 text-sm font-medium text-brand">Explore →</span></div>
          </Link>
        })}
      </div>
      {brands.length > 0 && <div className="mt-10"><div className="mb-4 flex items-end justify-between"><div><p className="text-xs font-semibold uppercase tracking-widest text-brand">Trusted systems</p><h3 className="mt-1 text-xl font-semibold">Shop by brand</h3></div><Link href="/brands" className="text-sm font-medium text-brand">All brands</Link></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">{brands.map((brand) => <Link key={brand.slug} href={`/b/${brand.slug}`} className="flex min-h-24 flex-col justify-between rounded-xl border border-border bg-card p-4 transition-colors hover:border-brand/50"><span className="text-base font-semibold">{brand.name}</span><span className="text-xs text-muted-foreground">{brand.productCount} products</span></Link>)}</div></div>}
    </section>
  )
}
...
export function CategoryDiscovery({ slug }: { slug: string }) {
  ...
  return <section className="mt-8 grid gap-4 lg:grid-cols-3"><div className="rounded-xl border border-border bg-card p-5"><p className="text-xs font-semibold uppercase tracking-widest text-brand">Relevant brands</p><div className="mt-4 flex flex-wrap gap-2">{(brands.length ? brands : commerce.brands.list().slice(0, 5)).map((brand) => <Link key={brand.slug} href={`/b/${brand.slug}`} className="rounded-full border border-border px-3 py-2 text-sm hover:border-brand">{brand.name}</Link>)}</div></div><div className="rounded-xl border border-border bg-card p-5"><p className="text-xs font-semibold uppercase tracking-widest text-brand">Systems and mounts</p><div className="mt-4 flex flex-wrap gap-2">{systems.map((system) => <Link key={system.id} href={`/shop?system=${system.slug}`} className="rounded-full border border-border px-3 py-2 text-sm hover:border-brand">{system.name}</Link>)}</div></div><div className="rounded-xl border border-border bg-card p-5"><p className="text-xs font-semibold uppercase tracking-widest text-brand">Accessories</p><div className="mt-4 flex flex-wrap gap-2">{groups.map((group) => <Link key={group.id} href={`/shop?accessory=${group.slug}`} className="rounded-full border border-border px-3 py-2 text-sm hover:border-brand">{group.name}</Link>)}</div></div></section>
}
...
export function BrandDiscovery({ slug }: { slug: string }) {
  ...
  return <section className="mt-8 rounded-xl border border-border bg-card p-5"><p className="text-xs font-semibold uppercase tracking-widest text-brand">Shop this brand by category</p><div className="mt-4 flex flex-wrap gap-2">{categories.map((category) => <Link key={category.slug} href={`/shop?brand=${brand.name}&category=${category.slug}`} className="rounded-full border border-border px-3 py-2 text-sm hover:border-brand">{category.name}</Link>)}</div></section>
}

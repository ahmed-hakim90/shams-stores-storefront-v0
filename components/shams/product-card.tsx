...
export function ProductCard({
  ...
  return (
    <article
      className={cn(
        cn('group mobile-product-card min-w-0 border border-border bg-card transition-[border-color,box-shadow,transform] duration-180 hover:border-brand/40 hover:shadow-[0_1px_0_rgba(0,0,0,0.02),0_12px_30px_-18px_rgba(21,63,112,0.35)]', view === 'list' ? 'relative grid min-h-[210px] grid-cols-[minmax(120px,38%)_minmax(0,1fr)] overflow-hidden rounded-xl sm:min-h-[210px] sm:grid-cols-[220px_minmax(0,1fr)]' : 'flex flex-col rounded-xl', view !== 'list' && 'mobile-product-card-grid'),
        className,
      )}
    >
      {/* MEDIA AREA */}
      <div className={cn('relative min-w-0 shrink-0 overflow-hidden bg-white', view === 'list' ? 'flex h-full flex-col border-r border-border p-3 sm:p-5' : view === 'rail' ? 'aspect-[1/1] p-3 sm:p-4' : 'aspect-[4/3] p-3 sm:aspect-[1.15/1] sm:p-5')}>
        {/* BADGES - Top Left */}
        ...
        {/* PRODUCT IMAGE */}
        <Link href={`/p/${product.slug}`} className="relative block min-h-0 w-full flex-1 overflow-hidden">
          <Image
            src={product.image || '/placeholder.svg'}
            alt={product.name}
            fill
            sizes={view === 'list' ? '(max-width: 639px) 38vw, 260px' : view === 'rail' ? '(max-width: 640px) calc(min(78vw, 292px) - 1.5rem), (max-width: 768px) calc(256px - 1.5rem), 240px' : '(max-width: 640px) calc(100vw - 40px), (max-width: 768px) calc(50vw - 60px), (max-width: 1280px) 190px, 220px'}
            className={cn(
      ...
      {/* CONTENT AREA */}
      <div className={cn('flex min-w-0 flex-1 flex-col gap-2 p-3 sm:gap-3 sm:p-5', view === 'grid' && 'border-t border-border', view === 'rail' && 'border-t border-border')}>
        {/* BRAND + RATING */}
        ...
        {view === 'list' && product.reviewCount > 0 && <RatingStars rating={product.rating} reviewCount={product.reviewCount} className="shrink-0" />}
        ...
        <div className="mt-auto flex min-w-0 flex-col gap-3 pt-1 sm:flex-row sm:items-end sm:justify-between">
          ...
          />
          <AddToCartButton product={product} className="w-full flex-none rounded-lg sm:w-auto sm:min-w-44" />
        </div>
      </div>
    </article>
  )

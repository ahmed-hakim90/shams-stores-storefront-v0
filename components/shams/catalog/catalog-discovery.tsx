    ...
    <section className={compact ? 'mb-4' : 'mx-auto max-w-[1440px] px-4 py-6 sm:px-6 sm:py-8'}>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
        {categories.map((category) => {
          ...
          return <Link key={category.slug} href={`/c/${category.slug}`} className="group relative min-h-24 overflow-hidden border border-border bg-card p-3 transition-colors duration-fast hover:border-foreground">

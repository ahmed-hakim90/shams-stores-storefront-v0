...
export function ProductDetail({ product }: { product: Product }) {
  ...
  return <main className="mx-auto max-w-[1400px] px-4 pb-24 pt-[calc(1.75rem+env(safe-area-inset-top))] sm:px-6 sm:py-10">
    ...
    {relationshipSections.length > 0 && <section className="mt-12 border-t border-border pt-8"><h2 className="text-2xl font-semibold">Build around this product</h2><div className="mt-6 grid gap-8">{relationshipSections.map((section) => <div key={section.type}><div className="flex items-center justify-between"><h3 className="font-semibold">{section.title}</h3><Link href={`/shop?relatedTo=${product.id}&relationship=${section.type}`} className="text-sm font-medium text-brand">View all</Link></div><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">{section.products.map((related) => <Link key={related.id} href={`/p/${related.slug}`} className="group rounded-lg border border-border bg-card p-3 transition-colors hover:border-brand/40"><div className="relative aspect-square rounded-md bg-white"><Image src={related.image} alt={related.name} fill sizes="160px" className="object-contain p-2 transition-transform duration-200 group-hover:scale-[1.03]" /></div><p className="mt-3 line-clamp-2 text-sm font-medium">{related.name}</p><p className="mt-1 text-sm font-semibold">{formatMoney(related.price)}</p></Link>)}</div></div>)}</div></section>}

...
export const commerce = {
        ...
        .filter(
          (p) =>
            ...
            (p.category === product.category ||
              p.useCases.some((useCase) => product.useCases.includes(useCase))),
        )
        .slice(0, 4),
  ...
  categories: {
    ...
    getBrands: (id: string) => {
      ...
      if (ids.length)
        return brands.filter((brand) => ids.includes(brand.id ?? brand.slug))
      const names = new Set(
        products
    ...
    getAccessoryGroups: (id: string) =>
      accessoryGroups.filter((group) => group.categoryIds.includes(id)),
    getProducts: (id: string, params: Omit<CatalogQuery, 'category'> = {}) =>
      commerce.products.page({ ...params, category: id }),
  },
  brands: {
    list: () => brands,
    ...
    getCategories: (id: string) => {
      ...
      if (ids.length)
        return categories.filter((category) =>
          ids.includes(category.id ?? category.slug),
        )
  ...
  systems: {
    list: (params?: { brandId?: string; categoryId?: string }) =>
      productSystems.filter(
        (system) =>
          (!params?.brandId || system.brandId === params.brandId) &&
          (!params?.categoryId ||
            system.categoryIds.includes(params.categoryId)),
      ),
    bySlug: (slug: string) =>
      productSystems.find((system) => system.slug === slug),
  ...
  relationships: {
    getForProduct: (productId: string, type?: ProductRelationshipType) =>
      productRelationships
        .filter(
          (relationship) =>
    ...
    summaries: (productId: string, type?: ProductRelationshipType, limit = 8) =>
      commerce.products.byIds(
        commerce.relationships
          .getForProduct(productId, type)
          .slice(0, limit)
  ...
  bundles: {
    ...
    getBySlug: (slug: string): BundleDetail | undefined => {
      ...
      const productsForBundle = commerce.products.byIds(
        bundle.items.map((item) => item.productId),
      )
      const statuses = bundle.items.map(
        (item) =>
          productsForBundle.find((product) => product.id === item.productId)
            ?.stock ?? 'out_of_stock',
      )
      const availability: BundleAvailability = statuses.some(
        (status) => status === 'out_of_stock',
      )
        ? 'partially_available'
        : statuses.some((status) => status === 'low_stock')
          ? 'low_stock'
          : statuses.some((status) => status === 'preorder')
            ? 'preorder'
            : 'available'
      return { ...bundle, availability, products: productsForBundle }
    },
    getFeatured: () => bundles.slice(0, 3),
    getByUseCase: (useCase: UseCaseSlug) =>
      bundles.filter((bundle) => bundle.useCase === useCase),
    ...
    validateSelection: (bundleId: string) => {
      ...
      const missingProductIds = bundle.items
    ...
    withProducts: (): BundleDetail[] =>
      bundles
        .map((bundle) => commerce.bundles.getBySlug(bundle.slug))
...
> = {
  unknown: { label: 'Check availability', tone: 'muted' },
